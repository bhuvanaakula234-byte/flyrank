...
This repository contains the completed **BE-04 Containerize Your Stack** assignment for the Backend AI Engineering track at FlyRank (Week 3).
...
Fetch all items again to verify that the newly added item survived container destruction:

```bash
curl http://localhost:3000/api/items
```

**Result:** The created item is still present in the response. Because PostgreSQL data is stored in the Docker volume `postgres_data`, data persists even when containers are stopped, removed, or rebuilt.
